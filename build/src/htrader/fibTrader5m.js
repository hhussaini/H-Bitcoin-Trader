...
let cancelled = false;
let initial10Triggered = false;
let initial10TradeSide = "";
let bigCandle = false;
...
function checkBook(book) {
    if (Date.now() - global5MinuteCounter < 300000) {
        ...
        if (bigCandle == false) {
            for (let i = 0; i < initialOrder10.length; i++) {
                gdaxAPI.cancelOrder(initialOrder10[i]).then((res) => {
...
//# sourceMappingURL=fibTrader5m.js.map