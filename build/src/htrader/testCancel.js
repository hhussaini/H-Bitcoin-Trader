...
let initialCancelBool = false;
let initial10ExecutedBool = false;
let cancelled = false;
let submitted = false;
let cancelCheck1 = false;
let global1SecondCounter = Date.now();
let lowestSell = 9999999;
let highestBuy = 0;
...
function checkBook(book) {
    ...
    if (Date.now() - global1SecondCounter >= 500000000000) {
        if (initialOrder10 != null && initial10ExecutedBool == false && cancelCheck1 == false) {
            if (initialOrder10.side === 'sell' && initialOrder10.price.toNumber().toFixed(2) != book.state().asks[0].price.toNumber().toFixed(2)) {
                ...
                gdaxAPI.cancelOrder(initialOrder10.id).then((res) => {
    ...
    if (Date.now() - global5MinuteCounter < 3000000000000000000) {
        ...
        else if ((open > close) && ((open - close) > 40)) {
            // gdaxAPI.cancelAllOrders(product).then((res: string[]) => {
            //     console.log(`\n CANCELLED ALL`);
            // }); // cancelling orders when another large graph is seen
...
//# sourceMappingURL=testCancel.js.map