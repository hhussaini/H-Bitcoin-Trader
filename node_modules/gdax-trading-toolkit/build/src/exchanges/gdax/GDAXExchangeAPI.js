...
class GDAXExchangeAPI {
    ...
    loadHistoricRates(options) {
        const url = `${this.apiURL}/products/BTC-USD/candles`;
        return request.get(url)
            .accept('application/json')
            .query({ granularity: options.granularity, start: options.start, end: options.end })
            .then((res) => {
            if (res.status !== 200) {
                throw new Error('loadHistoricRates did not get the expected response from the server. ' + res.body);
            }
            const orders = res.body;
