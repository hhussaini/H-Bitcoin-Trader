...
import { PlaceOrderMessage, StreamMessage, TradeFinalizedMessage, TradeExecutedMessage, OrderDoneMessage, NewOrderMessage} from "gdax-trading-toolkit/build/src/core";
...
let newOrder: NewOrderMessage;

let cancelBool: boolean = false;
let initialCancelBool: boolean = false;
...
let cancelCheck1: boolean = false;

let global1SecondCounter: number = Date.now();

let lowestSell: number = 9999999;
let highestBuy: number = 0;
...
// Configure the live book object
    ...
    feed.on('data', (msg: StreamMessage) => {
        ...
        if (msg.type === 'level'){
            ...
            newOrder = msg as NewOrderMessage;

            if (newOrder.side === 'buy' && parseInt(newOrder.price) > book.state().bids[0].price.toNumber()){
                //console.log("SOTP ORDER FOR BUYING");
            }
            else if (newOrder.side === 'sell' && parseInt(newOrder.price) < book.state().asks[0].price.toNumber()){
               // console.log("SOTP ORDER FOR SELLING");
            }
        }
    });
...
function submitStopTrade(side: string, amount: string, price: string, percent: string, stopPrice: string, stop: string) {
        ...
        gdaxAPI.placeOrder(order).then((result: LiveOrder) => {
            ...
            console.log(' \n Order executed', `INITIAL Order to ${order.side} 0.005 placed. Result: ID OF ${percent} is ${result.id} ${result.status} AND ${result.extra} AND ${result.price}`);
            pushMessage('Price Trigger', `INITIAL Order to ${order.side} 0.005 placed. Result: ID OF ${percent} is ${result.id} ${result.status}`);
        });
...
function checkBook(book: LiveOrderbook){ 
    ...
    console.log("HERE");
        ...
        if (submitted == false && Date.now() - global1SecondCounter >= 500000000000){

            let maxVolume2 = 0;
            let bidToChoose2 = 0;
            for (let i = 1; i < 7; i++){
                if (book.state().asks[i].totalSize.toNumber() > maxVolume2){
                    maxVolume2 = book.state().asks[i].totalSize.toNumber();
                    bidToChoose2 = i;
                }
            }
           

            if (book.state().asks[bidToChoose2].price.sub(book.state().asks[bidToChoose2-1].price).toNumber() > .01){
               
                let newPrice11 = book.state().asks[bidToChoose2].price.toNumber() + 0.01;
                let newPrice22 = newPrice11 + 10; 
                newPrice11 = book.state().asks[3].price.toNumber();
                console.log("\n BUY STOP PRICE IS: " + newPrice11.toFixed(2).toString());
                console.log("\n BUY LIMIT PRICE IS: " + newPrice22.toFixed(2).toString());
                submitStopTrade('buy', '0.0001', newPrice22.toFixed(2).toString(), '10.0', newPrice11.toFixed(2).toString(), 'entry');
            }
            ...
            if (book.state().bids[bidToChoose-1].price.sub(book.state().bids[bidToChoose].price).toNumber() > .01){
                
                let newPrice1 = book.state().bids[bidToChoose].price.toNumber() + 0.01;
                let newPrice2 = newPrice1 - 10; 
                newPrice1 = book.state().bids[3].price.toNumber();
                console.log("\n SELL STOP PRICE IS: " + newPrice1.toFixed(2).toString());
                console.log("\n SELL LIMIT PRICE IS: " + newPrice2.toFixed(2).toString());
                submitStopTrade('sell', '0.0001', newPrice2.toFixed(2).toString(), '38.2', newPrice1.toFixed(2).toString(), 'loss');
            }
            
            submitted = true;
        }

        if (Date.now() - global1SecondCounter >= 500000000000){
        if (initialOrder10 != null && initial10ExecutedBool == false && cancelCheck1 == false){
            if (initialOrder10.side === 'sell' && initialOrder10.price.toNumber().toFixed(2) != book.state().asks[0].price.toNumber().toFixed(2)){
                console.log('SELL CANCEL HERE');
                console.log(` \n THIS IS ORDER VAL: ${initialOrder10.price.toNumber().toFixed(2)}`);
                console.log(`THIS IS BOOK  VAL: ${book.state().asks[0].price.toNumber().toFixed(2)}`);
                // gdaxAPI.cancelAllOrders(product);
                
                // console.log(`\n CANCELLED ALL`);
                // submitTrade('sell', '0.0005', book.state().asks[0].price.toNumber().toFixed(2).toString(), '10.0');
                 // cancelling orders when another large graph is seen
                gdaxAPI.cancelOrder(initialOrder10.id).then((res: string) => {
                    console.log(`THIS IS WHAT WAS CANCELLED: ${res}`);
                    cancelCheck1 == true;
                    submitTrade('sell', '0.0005', book.state().asks[0].price.toNumber().toFixed(2).toString(), '10.0');
                }).catch(logError);
                
            }
            else if (initialOrder10.side === 'buy' && initialOrder10.price.toNumber().toFixed(2) != book.state().bids[0].price.toNumber().toFixed(2)){
                console.log('BUY CANCEL HERE');
                console.log(` \n THIS IS ORDER VAL: ${initialOrder10.price.toNumber().toFixed(2)}`);
                console.log(`THIS IS BOOK  VAL: ${book.state().bids[0].price.toNumber().toFixed(2)}`);
                // gdaxAPI.cancelAllOrders(product);
                    
                //         console.log(`\n CANCELLED ALL`);
                //         submitTrade('buy', '0.0005', book.state().bids[0].price.toNumber().toFixed(2).toString(), '10.0');
                // cancelling orders when another large graph is seen
                gdaxAPI.cancelOrder(initialOrder10.id).then((res: string) => {
                    console.log(`THIS IS WHAT WAS CANCELLED: ${res}`);
                    cancelCheck1 == true;
        ...
        global1SecondCounter = Date.now();
    }

   
    
    
    if (Date.now() - global5MinuteCounter < 3000000000000000000){
       
        

        let state = book.state(); 
        
        let buying = state.bids[0];
        let selling = state.asks[0];
        
        let buyingSize = buying.totalSize.toNumber();   //how many people trying to buy
        let sellingSize = selling.totalSize.toNumber(); //how many people trying to sell
        
        let buyingPrice = buying.price.toNumber(); //will always be 1 cent apart 
        let sellingPrice = selling.price.toNumber();
        ...
        if (buyingSize > 10000000){    //just for testing so typescript doesnt give errors
            ...
            submitStopTrade('buy', '0.02', '9237', '38.2', '032842', 'lol');
            console.log(printOrderbook(book, 5));
            printOrderbookStats(book);
            orderDone = null;
        ...
        if ((close > open) && ((close - open) > 40)){ //green bar, price went up

            // gdaxAPI.cancelAllOrders(product).then((res: string[]) => {
            ...
            pushMessage('BUY 38.2% ORDER', `Going to submit limit buy at $${ret2}`); //buy order need to buy before sell otherwise dont sell
            pushMessage('SELL 38.2% ORDER', `Going to submit limit sell at $${close}`); //sell order
            ...
            //this is extension 10.0%
            pushMessage('SELL 10.0% ORDER', `Going to submit limit sell at $${book.state().asks[0].price.toNumber()}`); //if it goes down straight
            pushMessage('BUY 10.0% ORDER', `Going to submit limit buy at $${ret5}`); //buy

            submitTrade('sell', '0.005', book.state().asks[0].price.toNumber().toString(), '10.0');
            ...
            console.log('BUY ORDER', `Going to submit limit buy at $${ret2}`); //buy order need to buy before sell otherwise dont sell
            console.log('SELL ORDER', `Going to submit limit sell at $${close}`); //sell order
            console.log('SELL ORDER', `Going to submit limit sell at $${ret1}`); //if it goes up slightly then goes down
            console.log('BUY ORDER', `Going to submit limit buy at $${ret5}`); //buy
            console.log('SELL ORDER', `Going to submit limit sell at $${ext5}`); //if it goes up only

            
         
        }

        else if ((open > close) && ((open - close) > 40)){ //red bar, price went down
            ...
            //this is retracement 38.2%
            pushMessage('SELL 38.2% ORDER', `Going to submit limit sell at $${ret2}`); //sell order need to sell before buy otherwise dont buy
            pushMessage('BUY 38.2% ORDER', `Going to submit limit buy at $${close}`); //buy order
            ...
            //this is extension 10.0%
            pushMessage('BUY 10.0% ORDER', `Going to submit limit buy at $${book.state().bids[0].price.toNumber()}`); //if it goes up straight
            pushMessage('SELL 10.0% ORDER', `Going to submit limit sell at $${ret5}`); //sell

            submitTrade('buy', '0.005', book.state().bids[0].price.toNumber().toString(), '10.0');
            ...
            //Print Statements 
            console.log('SELL ORDER', `Going to submit limit sell at $${ret2}`); //sell order need to sell before buy otherwise dont buy
            console.log('BUY ORDER', `Going to submit limit buy at $${close}`); //buy order
            console.log('BUY ORDER', `Going to submit limit sell at $${ret1}`); //if it goes down slightly then goes up
            console.log('SELL ORDER', `Going to submit limit buy at $${ret5}`); //buy     
            console.log('BUY ORDER', `Going to submit limit sell at $${ext5}`); //if it goes down only
...
function logError(err: any): void {
    console.log(err.message, err.response ? `THIS IS THE CACNELORDER ERROR ${err.response.status}: ${err.response.body.message}` : 'IT DOESNT KNOW');
}
